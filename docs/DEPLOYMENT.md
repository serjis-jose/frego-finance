...
1. Create namespace:
```bash
kubectl create namespace frego-finance
```
...
3. Deploy:
```bash
kubectl apply -f k8s/ -n frego-finance
```
...
```bash
kubectl scale deployment finance-service --replicas=5 -n frego-finance
```
